...
% 加一个 Lead 补偿器 C(s)=(s+0.5)/(s+5)：零点把轨迹往左吸，极点保证可实现
C_lead = tf([1, 0.5], [1, 5]);  % 零点 -0.5，极点 -5（proper，无零极点对消）
sys_comp = C_lead * sys;
subplot(1,2,2);
rlocus(sys_comp); grid on;
sgrid(0.7, 3);
title('加 Lead 补偿器后：根轨迹被"吸"向左');
hold off;

fprintf('【补偿器设计演示】\n');
fprintf('  原始系统: 根轨迹偏右 → 响应慢\n');
fprintf('  加 Lead 补偿器 (s+0.5)/(s+5): 零点吸轨迹向左 → 更快更稳定\n');
